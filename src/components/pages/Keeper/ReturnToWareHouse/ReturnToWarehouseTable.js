...
import {
  getRTWOrder,
  getkeeperOrdersReceive,
  receiveRTWOrders,
} from "../../../../store/AsyncMethod/KeeperMethod";
import { SET_RTW_ORDERS } from "../../../../store/Types/KeeperTypes";
import CustomPaginateTemplate from "../../../OtherComponents/CustomPaginateTempelate";

export default function ReturnToWarehouseTable() {
  const { t } = useTranslation();
  const { rtwOrders } = useSelector((state) => state.KeeperReducer);
  ...
  const handleReceiveChange = () => {
    if (!selectedOrders || selectedOrders.length === 0) {
      ...
      // dispatch(receiveRTWOrders(orders));
      // setselectedOrders([]);
      {
        const receivedOrderCodes = selectedOrders.map(
          (order) => order.order_code
        );
        dispatch(receiveRTWOrders(receivedOrderCodes));

        const updatedOrders = rtwOrders.filter(
          (order) => !receivedOrderCodes.includes(order.order_code)
        );

        dispatch({ type: SET_RTW_ORDERS, payLoad: updatedOrders });
        // setselectedOrders([]);
      }
    }
  };
      ...
      <div className="flex gap-2 pb-2">
        <span className="flex w-10/12 sm:w-8/12 md:w-6/12 lg:w-4/12 xl:w-3/12 ">
          <InputText
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                dispatch(getRTWOrder(parseInt(selectedInput)));
    ...
    <div>
      ...
      <div className="">
        <DataTable
          ...
          rows={5}
          paginatorLeft={left}
          paginatorTemplate={CustomPaginateTemplate()}
          filters={filters}
