...
import { getkeeperDestination } from "../../../../../store/AsyncMethod/KeeperMethod";
import { Toast } from "primereact/toast";
import { Dialog } from "primereact/dialog";
import DestinationOrders from "../DestinationOrders";
import { useNavigate } from "react-router-dom";
import { ConfirmDialog } from "primereact/confirmdialog";
import { RESET_KEEPER_ORDERS_DESTINATION } from "../../../../../store/Types/KeeperTypes";
import CustomPaginateTemplate from "../../../../OtherComponents/CustomPaginateTempelate";

export default function DestinationOrderTable() {
  ...
  useEffect(() => {
    if (keeperOrdersDestination) {
      setTotalRecords(totalRecords + 1);
    }
  }, [keeperOrdersDestination]);
  useEffect(() => {
    if (keeperOrdersDestination.length > 0) {
  ...
  const deleteRecords = () => {
    dispatch({ type: RESET_KEEPER_ORDERS_DESTINATION });
    setConfirmVisible(false);
  };

  const handleDistinationChange = () => {
      ...
      <div className="flex gap-2 pb-2">
        <span className="flex w-10/12 sm:w-8/12 md:w-6/12 lg:w-4/12 xl:w-3/12 ">
          <InputText
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                dispatch(getkeeperDestination(parseInt(selectedInput)));
