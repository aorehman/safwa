...
import { getkeeperOrdersReturn } from "../../../../../store/AsyncMethod/KeeperMethod";
import i18next from "i18next";
import { FilterMatchMode } from "primereact/api";
import AssignForReturn from "../AssignForReturn";
import CustomPaginator from "../../../../OtherComponents/customPaginator";

export default function ReturnOrderTable() {
  const { t } = useTranslation();
  const { keeperOrdersReturn } = useSelector((state) => state.KeeperReducer);
  ...
  useEffect(() => {
    if (keeperOrdersReturn) {
      setTotalRecords(keeperOrdersReturn.total);
    }
  }, [keeperOrdersReturn]);
  ...
  useEffect(() => {
    if (user) {
      dispatch(getkeeperOrdersReturn(page, rows));
    }
  }, [page, rows]);
  return (
    <div>
