...
import { getkeeperOrigin } from "../../../../../store/AsyncMethod/KeeperMethod";
import { Toast } from "primereact/toast";
import { Dialog } from "primereact/dialog";
import OriginOrders from "../OriginOrders";
import { useNavigate } from "react-router-dom";
import { RESET_KEEPER_ORDERS_ORIGIN } from "../../../../../store/Types/KeeperTypes";
import { ConfirmDialog } from "primereact/confirmdialog";
import CustomPaginateTemplate from "../../../../OtherComponents/CustomPaginateTempelate";

export default function OriginOrderTable() {
  const { t } = useTranslation();
  const { keeperOrdersOrigin } = useSelector((state) => state.KeeperReducer);
  ...
  useEffect(() => {
    if (keeperOrdersOrigin) {
      setTotalRecords(totalRecords + 1);
    }
  }, [keeperOrdersOrigin]);
  useEffect(() => {
    if (keeperOrdersOrigin.length > 0) {
  ...
  const deleteRecords = () => {
    dispatch({ type: RESET_KEEPER_ORDERS_ORIGIN });
    setConfirmVisible(false);
  };
      ...
      <div className="flex gap-2 pb-2">
        <span className=" flex w-10/12 sm:w-8/12 md:w-6/12 lg:w-4/12 xl:w-3/12 ">
          <InputText
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                dispatch(getkeeperOrigin(parseInt(selectedInput)));
