...
import { getkeeperOrders } from "../../../../../store/AsyncMethod/KeeperMethod";
...
export default function PickupOrdersTable() {
  ...
  const { keeperOrders } = useSelector((state) => state.KeeperReducer);
  ...
  useEffect(() => {
    if (keeperOrders) {
      setTotalRecords(keeperOrders.total);
    }
  }, [keeperOrders]);
