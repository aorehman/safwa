...
export default function KeeperListTable({ page, setPage, rows, setRows }) {
  const { t } = useTranslation();
  const [dialogVisible, setDialogVisible] = useState(false);
  const [selectedRow, setSelectedRow] = useState(null);
  const { user } = useSelector((state) => state.AuthReducer);
  const { keeperList } = useSelector((state) => state.AdminReducer);
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteKeeper(idsArray)).then((success) => {
          if (success) {
  ...
  useEffect(() => {
    if (keeperList) {
      setTotalRecords(keeperList.total);
    }
  }, [keeperList]);
