...
export default function ProvincesTable({ page, setPage, rows, setRows }) {
  const { t } = useTranslation();
  const { user } = useSelector((state) => state.AuthReducer);
  const { provinceList } = useSelector((state) => state.AdminReducer);
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteProvince(idsArray)).then((success) => {
          if (success) {
            setSelectedProducts(null);
            dispatch(getProvinceList(page, rows));
          }
        });
      }
    } else {
  ...
  useEffect(() => {
    if (provinceList) {
      setTotalRecords(provinceList.total);
    }
  }, [provinceList]);
