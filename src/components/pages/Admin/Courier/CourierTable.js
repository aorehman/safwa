...
function CourierTable({ page, setPage, rows, setRows }) {
  const { t } = useTranslation();
  const [dialogVisible, setDialogVisible] = useState(false);
  const [selectedRow, setSelectedRow] = useState(null);
  const { user } = useSelector((state) => state.AuthReducer);
  const { courierList } = useSelector((state) => state.AdminReducer);
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteCourier(idsArray)).then((success) => {
          if (success) {
            setSelectedProducts(null);
            dispatch(getCourierList(page, rows));
          }
        });
      }
    } else {
  ...
  useEffect(() => {
    if (courierList) {
      setTotalRecords(courierList.total);
    }
  }, [courierList]);
