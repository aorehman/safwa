...
import CourierPersonalDetail from "./PersonalDetails";
...
export default function AddCourier({
  ...
  const { specificKeeper, loading } = useSelector(
  ...
  const formik = useFormik({
    ...
    onSubmit: async (data) => {
      ...
      if (selectedRow) {
        ...
        dispatch(editCourier(updateData)).then((success) => {
          if (success) {
            formik.resetForm();
        ...
        dispatch(addCourier(data)).then((success) => {
          if (success) {
            formik.resetForm();
            setDialogVisible(false);
            setSelectedRow(null);
  ...
  return (
    <Dialog
      header={selectedRow ? "Edit Courier" : "Add Courier"}
        ...
        <form onSubmit={formik.handleSubmit} className=" ">
          ...
          >
            <div className="card pb-2 px-4  rounded-lg">
              <CourierPersonalDetail formik={formik} />
            </div>
