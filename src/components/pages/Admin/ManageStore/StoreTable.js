...
export default function StoreTable({ page, setPage, rows, setRows }) {
  const [showForm, setShowForm] = useState(false);
  const { t } = useTranslation();
  const { user } = useSelector((state) => state.AuthReducer);
  const { storeList } = useSelector((state) => state.AdminReducer);
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteStore(idsArray)).then((success) => {
          if (success) {
            setSelectedProducts(null);
            dispatch(getStoreList(page, rows));
          }
        });
      }
    } else {
  ...
  useEffect(() => {
    if (storeList) {
      setTotalRecords(storeList.total);
    }
  }, [storeList]);
      ...
      <div>
        <div className="px-1">
          <DataTable
            ...
            onRowClick={(e) => {
              // navigate("/admin-edit-store/" + e.data.id);
          ...
          >
            ...
            <Column
              ...
              header={t("City")}
              style={{ minWidth: "150px" }}
              body={(rowData) => rowData?.city?.name_en}
            />
            ...
            <Column
              ...
              header={t("Zone")}
              style={{ minWidth: "150px" }}
              body={(rowData) => rowData?.zone?.name_en}
            />
