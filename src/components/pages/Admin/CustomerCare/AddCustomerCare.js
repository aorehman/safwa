...
export default function AddCustomerCare({
  setDialogVisible,
  dialogVisible,
  selectedRow,
  setSelectedRow,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (data) => {
      ...
      if (selectedRow) {
        ...
        dispatch(editCustomerCare(updateData)).then((success) => {
          if (success) {
            formik.resetForm();
            setDialogVisible(false);
            dispatch(getCareList(1, 10));
            setSelectedRow(null);
            setSelectedProducts(null);
          }
        });
      } else {
        dispatch(addCustomerCare(data)).then((success) => {
          if (success) {
            formik.resetForm();
            setDialogVisible(false);
            dispatch(getCareList(1, 10));
  ...
  return (
    <Dialog
      header={selectedRow ? "Edit Customer Care" : "Add Customer Care"}
