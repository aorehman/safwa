...
import AddCourier from "../Courier/AddCourier";
import { useDispatch, useSelector } from "react-redux";
import AddCustomerCare from "./AddCustomerCare";
import { ConfirmDialog } from "primereact/confirmdialog";
import { Toast } from "primereact/toast";

function CustomerCareTable({ page, setPage, rows, setRows }) {
  const { t } = useTranslation();
  const [dialogVisible, setDialogVisible] = useState(false);
  const [selectedRow, setSelectedRow] = useState(null);
  const { user } = useSelector((state) => state.AuthReducer);
  const { customerCareList } = useSelector((state) => state.AdminReducer);
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteCustomerCare(idsArray)).then((success) => {
          if (success) {
            setSelectedProducts(null);
            dispatch(getCareList(page, rows));
          }
        });
      }
    } else {
  ...
  useEffect(() => {
    if (customerCareList) {
      setTotalRecords(customerCareList.total);
    }
  }, [customerCareList]);
