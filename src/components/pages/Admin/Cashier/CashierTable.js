...
function CashierTable({ page, setPage, rows, setRows }) {
  const { t } = useTranslation();
  const [dialogVisible, setDialogVisible] = useState(false);
  const [selectedRow, setSelectedRow] = useState(null);
  const { user } = useSelector((state) => state.AuthReducer);
  const { cashierList } = useSelector((state) => state.AdminReducer);
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteCashier(idsArray)).then((success) => {
          if (success) {
            setSelectedProducts(null);
            dispatch(getCashierList(page, rows));
          }
        });
      }
    } else {
  ...
  useEffect(() => {
    if (cashierList) {
      setTotalRecords(cashierList.total);
    }
  }, [cashierList]);
