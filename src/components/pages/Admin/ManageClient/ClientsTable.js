...
export default function ClientsTable({ page, setPage, rows, setRows }) {
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteClient(idsArray)).then((success) => {
          if (success) {
            setSelectedProducts(null);
            dispatch(getClientList(page, rows));
          }
        });
      }
    } else {
  ...
  useEffect(() => {
    if (clientList) {
      setTotalRecords(clientList.total);
    }
  }, [clientList]);
