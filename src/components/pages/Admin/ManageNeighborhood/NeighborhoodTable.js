...
export default function NeighborhoodTable({ page, setPage, rows, setRows }) {
  const { t } = useTranslation();
  const [dialogVisible, setDialogVisible] = useState(false);
  const [selectedRow, setSelectedRow] = useState(null);
  const { user } = useSelector((state) => state.AuthReducer);
  const { neighborhoodList } = useSelector((state) => state.AdminReducer);
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts.length !== 0) {
        dispatch(deleteNeighborhood(idsArray)).then((success) => {
          if (success) {
            setSelectedProducts(null);
            dispatch(getNeighborhoodList(page, rows));
          }
        });
      }
    } else {
  ...
  useEffect(() => {
    if (neighborhoodList) {
      setTotalRecords(neighborhoodList.total);
    }
  }, [neighborhoodList]);
